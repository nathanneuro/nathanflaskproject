...
def norm_df (df, weights, targets):
    df['norm_income'] = df['gallup.median.income']*weights[0]
    ...
    if len(weights) == 10:
        ...
        df['weight_score'] = (df['gallup.median.income']*weights[0] + df['gdp_obs']*weights[1] + df['gdp_proj']*weights[2] + df['digi_read']*weights[3] + df['digi_math']*weights[4] + df['pisa_math']*weights[5] + df['pisa_read']*weights[6] + df['pisa_sci']*weights[7] + df['top_mathers']*weights[8] + df['citi_score']*weights[9])/10
    else:
        df['weight_score'] = (df['gallup.median.income']*weights[0] + df['gdp_obs']*weights[1] + df['gdp_proj']*weights[2] + df['digi_read']*weights[3] + df['digi_math']*weights[4] + df['pisa_math']*weights[5] + df['pisa_read']*weights[6] + df['pisa_sci']*weights[7] + df['top_mathers']*weights[8])/9
...
def line_plot(dfline, subjects, factors, give_title):
    factors.append('Weight Score')
    print(factors)
    plot = pygal.Line(x_label_rotation=20, stroke_style={'width': 3}, margin=10, y_title='Factor as percent of Mean * Weight')
    plot.x_labels = factors

    for subject in range(subjects):
        row_values = dfline.iloc[subject,]
        row_values = row_values.values
        #print("Row Values: ", row_values)
        #cols = df.columns.tolist()
        #print(cols[:])
        #print(dfline.index[subject])
        plot.add(dfline.index[subject], row_values)
    return plot.render()
